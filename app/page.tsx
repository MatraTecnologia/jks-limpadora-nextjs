...
export default function Home() {
  ...
  return (
    <>
      <section className="relative flex min-h-[86vh] items-end overflow-hidden bg-ink text-white">
        {hero ? (
        ...
        <div className="container-x relative grid gap-10 pt-24 pb-14 sm:pb-20 lg:grid-cols-[1fr_auto] lg:items-end">
          <div className="grid max-w-3xl justify-items-start gap-6">
            <p className="label-rule text-sun before:bg-sun animate-in fade-in slide-in-from-bottom-3 duration-700">Limpeza técnica especializada · desde 1990</p>
            <h1 className="display text-[clamp(2.9rem,7vw,6rem)] animate-in fade-in slide-in-from-bottom-4 delay-75 duration-700 fill-mode-both">
      ...
      <section className="border-b border-border">
        <dl className="container-x grid grid-cols-2 lg:grid-cols-4">
          {facts.map((f, i) => (
            <div key={f.label} className={`grid gap-1 py-6 ${i > 0 ? "lg:border-l lg:border-border lg:pl-8" : ""} ${i % 2 === 1 ? "border-l border-border pl-6 lg:pl-8" : ""}`}>
              <dt className="text-xs font-bold tracking-[0.1em] text-muted-foreground uppercase">{f.label}</dt>
              <dd className="text-xl font-extrabold tracking-tight sm:text-2xl">{f.value}</dd>
            </div>
          ))}
        </dl>
      </section>
      ...
      <section className="py-20 sm:py-28">
        <div className="container-x">
          ...
          <ol className="grid gap-8 border-t border-ink md:grid-cols-3 md:gap-6">
            {steps.map((s, i) => (
      ...
      <section id="simular" className="bg-paper py-20 sm:py-28">
