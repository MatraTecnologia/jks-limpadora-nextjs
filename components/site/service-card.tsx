...
export const ServiceCard = ({ service, image, index, large = false }: Props) => (
  <Link href={`/${service.slug}`} className="group relative block overflow-hidden rounded-xl bg-ink text-white">
    <div className={cn("relative overflow-hidden", large ? "aspect-[16/11]" : "aspect-[16/10] sm:aspect-[16/12]")}>
      {image ? (
    ...
    <div className="absolute inset-x-0 bottom-0 grid gap-1.5 p-4 sm:p-5">
      <h3 className={cn("leading-[1.05] font-extrabold tracking-tight", large ? "text-2xl sm:text-3xl" : "text-xl")}>{service.title}</h3>
      <p className={cn("max-w-[42ch] text-[0.875rem] leading-snug text-white/80", !large && "line-clamp-2 max-sm:hidden")}>{service.summary}</p>
    </div>
  </Link>
);
